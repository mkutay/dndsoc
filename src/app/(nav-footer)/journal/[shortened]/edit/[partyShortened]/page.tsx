...
export default async function Page({ params }: { params: Promise<{ shortened: string; partyShortened: string }> }) {
  ...
  if (user.isErr() && user.error.code !== "NOT_LOGGED_IN")
    return <ErrorComponent error={user.error} caller="/journal/[shortened]/edit/[partyShortened]/page.tsx" />;
  if (user.isErr()) {
    forbidden();
  }
  ...
  const result = await cachedGetPartyEntry({ journalShortened: shortened, partyShortened });
  if (result.isErr())
    return <ErrorComponent error={result.error} caller="/journal/[id]/edit/[partyShortened]/page.tsx" />;
  const entry = result.value;

  return (
    <div className="flex flex-col w-full mx-auto lg:max-w-6xl max-w-prose lg:my-12 mt-6 mb-12 px-4">
      <Link
        href={`/journal/${entry.journal.shortened}`}
