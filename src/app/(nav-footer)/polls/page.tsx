...
export default async function Page() {
  ...
  if (user.isErr() && user.error.code !== "NOT_LOGGED_IN")
    return <ErrorComponent error={user.error} caller="/polls/page.tsx" />;
      ...
      <div key={poll.id}>
        ...
        >
          <h2 className="md:text-4xl text-3xl font-medium tracking-normal font-headings flex flex-row items-start w-fit flex-wrap">
            ...
            {expired ? (
              <div className="font-quotes text-destructive hover:text-destructive/80 transition-all">EXPIRED</div>
            ) : null}
          </h2>
        </Link>
        <ul className="pl-5 space-y-1 mt-2">
          {poll.options.map((option) => (
            <li key={option.id} className="flex flex-row items-start gap-2">
              ...
              <div className="uppercase font-quotes">
                {option.text} — Votes: {option.votes[0].count}
              </div>
            </li>
          ))}
        </ul>
        <Button asChild variant="secondary" className="mt-4">
          <Link href={`/polls/${poll.shortened}`}>See More</Link>
        </Button>
      </div>,
    );
...
const getAllPolls = () =>
  runQuery((supabase) =>
    supabase.from("polls").select("*, options(*, votes(count))").order("created_at", { ascending: false }),
  );
