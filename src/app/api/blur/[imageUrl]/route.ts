...
export async function GET(request: NextRequest, { params }: { params: Promise<{ imageUrl: string }> }) {
  try {
    ...
    return NextResponse.json({ error: "Internal server error while generating blur-sm placeholder" }, { status: 500 });
  }
}
