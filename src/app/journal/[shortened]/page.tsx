...
export default async function Page({ params }: { params: Promise<{ shortened: string }> }) {
  ...
  if (partiesResult.isErr() && partiesResult.error.code !== "NOT_LOGGED_IN") return <ErrorComponent error={partiesResult.error} caller="/journal/[shortened]/page.tsx" />;
  ...
  for (let i = 0; i < journal.party_entries.length; i++) {
    ...
    partyEntries.push(
      <div key={entry.journal_id + entry.party_id} className="flex flex-col gap-2">
        {entry.text.length !== 0 ? <div className="max-w-prose text-lg items-start">
          <div className="text-5xl font-drop-caps float-start mr-2.5">{entry.text[0]}</div>
          {entry.text.slice(1)}
        </div> : <div className="text-lg italic text-muted-foreground">No entry for this party.</div>}
        <div className="flex flex-row items-center text-md italic font-quotes mt-2">
          —<Link href={`/parties/${entry.parties.shortened}`} className="text-foreground hover:text-foreground/80 transition-all underline">
            {entry.parties.name}
          </Link>
        </div>
        {partiesResult.isOk() && partiesResult.value.includes(entry.party_id) && <div className="flex flex-row justify-end w-full mt-2">
          <Button variant="outline" asChild className="w-fit">
            <Link href={`/journal/${journal.shortened}/edit/${entry.parties.shortened}`}>
              Edit Entry
            </Link>
          </Button>
        </div>}
  ...
  return (
    <div className="flex flex-col w-full mx-auto lg:max-w-6xl max-w-prose lg:my-12 mt-6 mb-12 px-4">
      ...
      {user.isOk() && user.value.role === "admin" && <div className="flex flex-row justify-end w-full mt-4">
        <Button variant="outline" asChild className="w-fit">
          <Link href={`/journal/${journal.shortened}/edit`}>
            Edit Journal
          </Link>
        </Button>
      </div>}
