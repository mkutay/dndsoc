  ...
  return (
    <div className="flex flex-col w-full mx-auto lg:max-w-6xl max-w-prose lg:my-12 mt-6 mb-12 px-4">
      <TypographyH1 className="flex flex-row flex-wrap">
        ...
        {expired && <div className="font-quotes text-destructive">EXPIRED</div>}
      </TypographyH1>
      <div className="text-right italic font-quotes text-lg mt-2">
        <div>Created at: {format(poll.created_at, "PP")}</div>
        {poll.expires_at && (expired ? <div>Expired on: {format(poll.expires_at, "PP")}</div> : <div>Expires at: {format(poll.expires_at, "PP")}</div>)}
      </div>
      {user.isOk() && user.value.role === "admin" && <div className="flex flex-row justify-end w-full mt-4">
        <Button variant="outline" asChild className="w-fit">
          <Link href={`/polls/${poll.shortened}/edit`}>
            Edit Poll
          </Link>
        </Button>
      </div>}
      <Options
        ...
        votedId={user.isOk() ? poll.options.find((option) => option.votes.some(vote => vote.auth_user_uuid === user.value.auth_user_uuid))?.id || null : undefined}
        pollId={poll.id}
