...
import { updateAdmin } from "@/server/admins";
...
export function AdminEditSheet({
  ...
  const onSubmit = (values: z.infer<typeof adminEditSchema>) => {
    startTransition(async () => {
      const result = actionResultToResult(await updateAdmin(values, path));
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 px-1">
              <SheetHeader>
                ...
                <SheetDescription>Edit your public admin page. This is visible to everyone.</SheetDescription>
              </SheetHeader>
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>Upload a new image for your admin profile.</FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              />
