...
export const voteOnPoll = async ({ optionId, pollId }: { optionId: string, pollId: string }) => resultAsyncToActionResult(
  getUser()
    .andThen((user) => runQuery((supabase) => supabase
      .rpc("vote_on", {
...
export const editPoll = async (pollId: string, data: z.infer<typeof editPollSchema>) => {
  ...
  const polls = parsed
    ...
    .andThen((result) => runQuery((supabase) =>
      supabase
        .from("options")
        .delete()
        .eq("poll_id", pollId)
